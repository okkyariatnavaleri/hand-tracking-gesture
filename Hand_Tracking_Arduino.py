...
cap.set(3, 1280) 
cap.set(4, 720)
detector = HandDetector(detectionCon=int(0.5), maxHands=1) # menentukan jumlah tangan yang dideteksi dalam box
...
while True:
    ...
    if lmList:
        ...
        # Info samping kiri
        cv2.putText(img, f'Jumlah jari: {totalFingers}', (20, 640), cv2.FONT_HERSHEY_PLAIN,
                1.3, (0, 255, 255), 2)
        ...
        cv2.putText(img, f'Tangan: {myHandType}', (20, 620), cv2.FONT_HERSHEY_PLAIN,
                1.3, (0, 255, 255), 2)
        
        # Menampilkan teks Okky Ariatna
        cv2.putText(img, 'Created by: Okky Ariatna', (20, 660), cv2.FONT_HERSHEY_PLAIN,
                1.3, (0, 255, 255), 2)
        ...
        cv2.putText(img, f'Lampu:{text}', (20, 600), cv2.FONT_HERSHEY_PLAIN,
                1.3, (255, 255, 0), 2) # box nampilin lampu mati atau hidup

    # Menampilkan nilai FPS
    fps, img = fpsReader.update(img,pos=(1130,650),color=(0,0,255),scale=2,thickness=3)

    # Display
    cv2.imshow("On/Off Lamp Using Hand Tracking, by: Okky Ariatna", img) # Menampilkan info di kotak deteksi
